...
struct BlobView: View {
    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            ...
            Canvas { context, size in
                context.fill(path(in: CGRect(x: 0, y: 0, width: size.width, height: size.height), x: x, x2: x2), with: .linearGradient(Gradient(colors: [.pink, .blue]), startPoint: CGPoint(x: 0, y: 0), endPoint: CGPoint(x: size.height, y: size.width)))
            }
    ...
    func path(in rect: CGRect, x: Double, x2: Double) -> Path {
        ...
        path.addCurve(to: CGPoint(x: 0.6355*width, y: height), control1: CGPoint(x: 0.92554*width, y: 0.77749*height*x2), control2: CGPoint(x: 0.91864*width*x2, y: height))
